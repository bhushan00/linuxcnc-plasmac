...
class materialConverter:
    ...
    def create_widgets(self):
        self.T = gtk.Table(5, 3)
        self.T.set_row_spacings(8)
        self.W.add(self.T)
        self.inLabel = gtk.Label('Input File')
        self.inLabel.set_alignment(0.95,0.5)
        self.T.attach(self.inLabel, 0, 1, 1, 2)
        self.outLabel = gtk.Label('Output File')
        self.inLabel.set_alignment(0.95,0.5)
        self.T.attach(self.outLabel, 0, 1, 2, 3)
        self.inFile = gtk.Entry()
        self.inFile.set_width_chars(60)
        self.T.attach(self.inFile, 1, 2, 1, 2)
        self.outFile = gtk.Entry()
        self.outFile.set_width_chars(60)
        self.T.attach(self.outFile, 1, 2, 2, 3)
        self.outMetric = gtk.RadioButton(group=None, label='Metric')
        self.T.attach(self.outMetric, 2, 3, 1, 2)
        self.outImperial = gtk.RadioButton(group=self.outMetric, label='Imperial')
        self.T.attach(self.outImperial, 2, 3, 2, 3)
        self.B = gtk.Table(1, 5, homogeneous=True)
        self.T.attach(self.B, 1, 2, 3, 4)
        self.convert = gtk.Button('Convert')
        self.B.attach(self.convert, 2, 3, 0, 1)
        self.outLabel = gtk.Label('')
        self.T.attach(self.outLabel, 2, 3, 3, 4)
    ...
    def on_metric_toggled(self,button,name):
        ...
        if button.get_active():
            ...
            self.divisor = 25.4
            self.precision = 3
        print 'divisor = %0.1f' % (self.divisor)
    ...
    def on_convert_clicked(self,button):
        ...
        if not os.path.exists(self.inFileName):
            print '%s does not exist' % (self.inFileName)
            return

        print 'convert from %s to %s...' % (self.inFileName,self.outFileName)
