...
w('label',fmonitor + '.lAlab','-text','THC Active')
...
w('label',fmonitor + '.lUlab','-text','Moving Up')
...
w('label',fmonitor + '.lDlab','-text','Moving Down')
...
w('grid',fmonitor + '.led-thc-active','-row','0','-column','2','-sticky','e')
w('grid',fmonitor + '.lAlab','-row','0','-column','3')
...
w('DynamicHelp::add',fmonitor + '.led-thc-active','-text','THC status indicator')
...
configure_widgets()
w(fpausedmotion + '.paused-motion-speed','set',inifile.find('PLASMAC','PAUSED-MOTION-SPEED') or '0.5')
w(ftorch + '.torch-pulse-time','set',inifile.find('PLASMAC','TORCH-PULSE-TIME') or '0.5')
